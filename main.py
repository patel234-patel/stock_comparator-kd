...
# Flow every 0.1s:
#   1. Poll prices from Angel One + Motilal WS/REST
#   2. Try push to Google Sheets → if True: sheet updated
#                                 → if False (throttled): write to Excel instead
#   3. Excel always has freshest data (xlwings = milliseconds, no HTTP)
#   4. Google Sheet catches up every 1.5s
# """
...
#     ENABLE_GOOGLE_SHEET, ENABLE_EXCEL,
...
#                 abs(a["angel_buy"]    - b["angel_buy"])    > 0.001 or
#                 abs(a["angel_sell"]   - b["angel_sell"])   > 0.001 or
#                 abs(a["motilal_buy"]  - b["motilal_buy"])  > 0.001 or
#                 abs(a["motilal_sell"] - b["motilal_sell"]) > 0.001):
#             return True
#     return False
...
#     print(f"  Symbols: {len(SYMBOLS)}  |  Poll: {REFRESH_INTERVAL}s  |  Sheet: {SHEET_WRITE_INTERVAL}s  |  Excel: every tick")
#     print("=" * 65)
...
#             # ── Fetch prices ───────────────────────────────────────────────────
#             angel_snap   = build_price_snapshot(angel,   SYMBOLS)
#             motilal_snap = build_price_snapshot(motilal, SYMBOLS)

#             if not angel_snap or not motilal_snap:
#                 print(
#                     f"\r  Waiting — angel:{len(angel_snap)}/{len(SYMBOLS)} "
...
#             # ── Always write to Excel (instant via xlwings COM/AppleScript) ───
#             if excel:
#                 try:
#                     excel.update(ranked)
#                     excel_writes += 1
...
#             top = ranked[0]
#             sheet_indicator = "✅" if sheet_pushed else "⏳"
#             print(
...
#                 f"  {sheet_indicator}Sheet  📊Excel:{excel_writes}"
#                 f"  [{elapsed:.0f}ms]",
#                 end="", flush=True,
#             )
